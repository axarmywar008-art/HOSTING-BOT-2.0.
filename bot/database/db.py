...
class Database:
    ...
    async def _create_indexes(self):
        ...
        await self.db.payments.create_index([("status", 1), ("created_at", -1)])
        await self.db.payments.create_index("user_id")
        await self.db.point_logs.create_index([("user_id", 1), ("timestamp", -1)])
        await self.db.hosting_history.create_index([("user_id", 1), ("created_at", -1)])
    ...
    async def create_user(self, user_id: int, username: str = "", full_name: str = "", referred_by: int = None):
        user = {
            ...
            "balance": 0,
            "points": settings.MIN_POINTS_TO_HOST,  # Starting points
            "language": "en",
            "notifications": True,
            "payment_state": None,
            "current_state": None,
    ...
    async def add_log(self, log_type: str, data: dict):
        log = {"type": log_type, "data": data, "timestamp": time.time()}
    ...
    # ---- Payment Operations ----
    async def create_payment_request(self, user_id: int, amount: float, upi_id: str, transaction_id: str, screenshot_file_id: str) -> dict:
        payment = {
            "payment_id": str(uuid.uuid4()),
            "user_id": user_id,
            "amount": amount,
            "points": int(amount * settings.POINTS_PER_RUPEE),
            "upi_id": upi_id,
    ...
    async def get_pending_payments(self, limit: int = 50) -> List[dict]:
        return await self.db.payments.find({"status": "pending"}).sort("created_at", 1).limit(limit).to_list(None)

    async def get_user_payments(self, user_id: int, limit: int = 20) -> List[dict]:
        return await self.db.payments.find({"user_id": user_id}).sort("created_at", -1).limit(limit).to_list(None)

    async def approve_payment(self, payment_id: str, admin_id: int) -> bool:
        payment = await self.get_payment_request(payment_id)
        ...
        points_to_add = payment.get("points", 0)
            ...
            {"user_id": user_id},
            {
                ...
                "$push": {
                    "transactions": {
                        ...
                        "description": f"Payment approved: {payment['transaction_id']}",
                        "timestamp": time.time(),
                    }
                }
            },
    ...
    async def reject_payment(self, payment_id: str, admin_id: int, reason: str) -> bool:
        payment = await self.get_payment_request(payment_id)
        if not payment or payment.get("status") != "pending":
            return False
    ...
    async def update_user_points(self, user_id: int, points: int, reason: str = "Admin adjustment") -> bool:
        result = await self.db.users.update_one(
            {"user_id": user_id},
            {"$set": {"points": points}}
        )
        if result.modified_count > 0:
            await self.db.point_logs.insert_one({
                "user_id": user_id,
                "new_points": points,
                "reason": reason,
                "timestamp": time.time(),
    ...
    async def deduct_points(self, user_id: int, points: int, reason: str = "Deployment") -> bool:
        user = await self.get_user(user_id)
        if not user:
            return False
    ...
    async def get_point_logs(self, user_id: int, limit: int = 20) -> List[dict]:
        return await self.db.point_logs.find({"user_id": user_id}).sort("timestamp", -1).limit(limit).to_list(None)

    # ---- Hosting History ----
    async def add_hosting_history(self, user_id: int, deployment_id: str, points_used: int, duration_hours: int):
        await self.db.hosting_history.insert_one({
            "user_id": user_id,
    ...
    async def get_hosting_history(self, user_id: int, limit: int = 20) -> List[dict]:
        return await self.db.hosting_history.find({"user_id": user_id}).sort("created_at", -1).limit(limit).to_list(None)

    async def update_hosting_status(self, deployment_id: str, status: str):
        await self.db.hosting_history.update_one(
            {"deployment_id": deployment_id},
            {"$set": {"status": status}}
        )
    ...
    async def get_expired_deployments(self) -> List[dict]:
        now = time.time()
        return await self.db.hosting_history.find({
            "expires_at": {"$lt": now},
            "status": "active"
        }).to_list(None)
