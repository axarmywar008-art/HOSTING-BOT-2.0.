...
async def get_deployment_from_callback(query, user_id: int):
    """Get deployment from callback data or user's active deployment."""
    data = query.data
    if len(data) >= 36:
...
async def callback_handler(client: Client, query: CallbackQuery):
    ...
    handlers = {
        ...
        # Points admin callbacks
        "admin_add_points": cb_admin_add_points,
        "admin_deduct_points": cb_admin_deduct_points,
        "admin_points_users": cb_admin_points_users,
        "admin_set_points": cb_admin_set_points,
        "admin_set_daily_cost": cb_admin_set_daily_cost,
        "admin_set_min_points": cb_admin_set_min_points,
        "points": cb_points,
        # Variable callbacks
        "var_add": cb_var_add,
    ...
    try:
        ...
        err_markup = InlineKeyboardMarkup([[btn("🏡 Main Menu", "main_menu")]])
        try:
...
async def cb_main_menu(client: Client, query: CallbackQuery):
    ...
    start_text = START_CAPTION.format(user=query.from_user.first_name or "user")
...
async def cb_deploy_menu(client: Client, query: CallbackQuery):
    ...
    await query.message.edit_text(
        ...
        f"<b>📉 Daily Cost:</b> {settings.DAILY_POINT_COST} pts/day\n\n"
...
async def cb_points(client: Client, query: CallbackQuery):
    """Show user's points balance and transaction history."""
    user_id = query.from_user.id
    points = await database.get_points_balance(user_id)
...
# ==================== ADMIN POINTS CALLBACKS ====================

async def cb_admin_add_points(client: Client, query: CallbackQuery):
...
async def cb_admin_deduct_points(client: Client, query: CallbackQuery):
    if query.from_user.id not in settings.OWNER_IDS:
        await query.answer("Unauthorized", show_alert=True)
        return
    await query.message.edit_text(
...
async def cb_admin_points_users(client: Client, query: CallbackQuery):
    if query.from_user.id not in settings.OWNER_IDS:
        await query.answer("Unauthorized", show_alert=True)
        return
...
async def cb_admin_set_points(client: Client, query: CallbackQuery):
    if query.from_user.id not in settings.OWNER_IDS:
        await query.answer("Unauthorized", show_alert=True)
        return
    await query.message.edit_text(
...
async def cb_admin_set_daily_cost(client: Client, query: CallbackQuery):
    if query.from_user.id not in settings.OWNER_IDS:
        await query.answer("Unauthorized", show_alert=True)
        return
    await query.message.edit_text(
...
async def cb_admin_set_min_points(client: Client, query: CallbackQuery):
    if query.from_user.id not in settings.OWNER_IDS:
        await query.answer("Unauthorized", show_alert=True)
        return
    await query.message.edit_text(
...
async def cb_my_bot(client: Client, query: CallbackQuery):
    ...
    if not dep:
        ...
        # Get point status
        point_status = await point_deduction_worker.get_deployment_point_status(dep["deployment_id"])
        points_remaining = point_status.get("points_remaining", "N/A")
        daily_cost = point_status.get("daily_cost", settings.DAILY_POINT_COST)
        ...
        if isinstance(points_remaining, int):
            days_remaining = points_remaining // daily_cost if daily_cost > 0 else 0
            remaining_text = f"{days_remaining} days" if days_remaining > 0 else "⚠ Low"
        else:
            remaining_text = "N/A"
        ...
        text = (
            ...
            f"<b>📉 Daily Cost:</b> {daily_cost} pts/day\n"
            f"<b>📅 Days Remaining:</b> {remaining_text}\n\n"
...
async def cb_confirm(client: Client, query: CallbackQuery):
    ...
    if action.startswith("stop"):
        ...
        # Deduct points BEFORE deployment
        points = await database.get_points_balance(user_id)
        min_points = settings.MINIMUM_POINTS_FOR_DEPLOYMENT
        ...
        if points < min_points:
            await query.message.edit_text(
                ...
                f"You need <b>{min_points}</b> points to deploy.\n"
                f"Your current balance: <b>{points}</b> points."
            )
            return
        
        success = await database.deduct_points(user_id, min_points, "deployment_cost")
        if not success:
            await query.message.edit_text("<b>❌ Failed to deduct points. Please try again.</b>")
            return
...
async def cb_admin_panel(client: Client, query: CallbackQuery):
    ...
    await query.message.edit_text(
        ...
        f"<b>📉 Daily Cost:</b> {settings.DAILY_POINT_COST} pts\n"
        f"<b>📌 Min Points:</b> {settings.MINIMUM_POINTS_FOR_DEPLOYMENT} pts\n\n"
...
async def cb_admin_api_health(client: Client, query: CallbackQuery):
    ...
    text += f"\n<b>Last check:</b> <code>{datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC</code>"
        ...
        try:
            for _ in range(15):
                ...
                try:
                    ...
                    if expected_kw not in msg.text:
                        break
                except Exception:
                    break
...
# ==================== BACKGROUND DEPLOYMENT TRACKING ====================

async def track_background_deployment(client: Client, message, user_id: int, context: dict, dep_vars: dict):
    """Track deployment progress in background."""
    logger = logging.getLogger(__name__)
    from bot.services.log_service import owner_log
    building_kb = InlineKeyboardMarkup([[btn("🏡 Main Menu", "main_menu")]])
            ...
            try:
                ...
                if status in ("SUCCESS", "RUNNING", "CRASHED", "FAILED"):
                    if status in ("SUCCESS", "RUNNING"):
                        ...
                        # Create point tracking record for this deployment
                        await database.create_deployment_point_record(
                            deployment_id, 
                            user_id, 
                            settings.DAILY_POINT_COST
                        )
                        ...
                        await owner_log.send_user_notification(
                            ...
                            f"<b>⭐ Points Remaining:</b> {await database.get_points_balance(user_id)}\n"
                            f"<b>📉 Daily Cost:</b> {settings.DAILY_POINT_COST} pts/day"
                        )
                        try:
                            current_msg = await client.get_messages(message.chat.id, message.id)
...
async def track_background_redeploy(client: Client, message, user_id: int, deployment_id: str, railway_deployment_id: str):
    """Track redeployment progress in background."""
    logger = logging.getLogger(__name__)
    try:
        dep = await database.get_deployment(deployment_id)
        if not dep:
