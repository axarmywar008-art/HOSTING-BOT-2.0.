...
DEAD_STATUSES = {"CRASHED", "FAILED", "REMOVED", "SLEEPING", "SKIPPED", "WAITING", "EXPIRED"}
...
class DeploymentWorker:
    ...
    async def start(self):
        ...
        asyncio.create_task(self._run_expiry_checker())  # NEW
        logger.info("Deployment worker started")

    async def stop(self):
        self._running = False

    # NEW: Check for expired deployments every 5 minutes
    async def _run_expiry_checker(self):
        while self._running:
            try:
    ...
    async def _check_expired_deployments(self):
        now = time.time()
        expired = await database.get_expired_deployments()
        
        for record in expired:
            dep = await database.get_deployment(record["deployment_id"])
            if not dep or dep.get("status") != "running":
                continue
            
            logger.info(f"Stopping expired deployment: {dep['deployment_id']}")
            await deployment_engine.stop_deployment(dep["deployment_id"])
            await database.update_deployment(dep["deployment_id"], {
                "status": "expired",
                "expired_at": now
            })
            ...
            await owner_log.send_user_notification(
                dep["user_id"],
                f"⏰ <b>Bot Expired</b>\n\n"
                f"Your bot <code>{dep['deployment_id'][:8]}</code> has expired.\n"
                f"<b>Duration:</b> 1 hour\n"
                f"<b>Points Used:</b> {dep.get('points_used', 0)}\n\n"
                f"<b>To renew, please deploy again with points.</b>"
            )
            
            # Release token
            await token_manager.release_token(dep["railway_token"])
            await database.update_hosting_status(dep["deployment_id"], "expired")
