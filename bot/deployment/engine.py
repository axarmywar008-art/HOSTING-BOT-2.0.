...
class DeploymentEngine:
    ...
    async def _check_points(self, user_id: int, hours: int = 1) -> tuple:
        """Check if user has enough points for deployment"""
        user = await database.get_user(user_id)
        if not user:
            return False, "User not found"
        
        points = user.get("points", 0)
        required_points = hours * settings.HOST_COST_POINTS
        
        if points < required_points:
            return False, f"Insufficient points. Required: {required_points}, You have: {points}"
        
        return True, points

    async def _deduct_points_for_deployment(self, user_id: int, deployment_id: str, hours: int = 1) -> bool:
        """Deduct points and record hosting history"""
        points_to_deduct = hours * settings.HOST_COST_POINTS
        
        success = await database.deduct_points(
            user_id, 
            points_to_deduct, 
            f"Bot deployment (ID: {deployment_id[:8]}) - {hours} hour(s)"
        )
        
        if success:
            await database.add_hosting_history(
                user_id,
                deployment_id,
                points_to_deduct,
                hours
    ...
    async def deploy_from_github(self, user_id: int, repo_url: str, variables: dict = None, scan_result: dict = None) -> dict:
        ...
        # --- CHECK POINTS ---
        hours = 1  # Default: 1 hour hosting
        has_points, points_info = await self._check_points(user_id, hours)
        if not has_points:
            terminal.add_error(f"Points check failed: {points_info}")
            return {"success": False, "error": points_info, "deployment_id": deployment_id}
        ...
        while retries < max_retries:
            ...
            try:
                ...
                # --- DEDUCT POINTS ---
                points_deducted = await self._deduct_points_for_deployment(user_id, deployment_id, hours)
                if not points_deducted:
                    terminal.add_error("Failed to deduct points")
                    await client.delete_project(project_id)
                    return {"success": False, "error": "Failed to deduct points", "deployment_id": deployment_id}
                ...
                dep_record = {
                    ...
                    "restart_count": 0,
                    "total_runtime": 0,
                    "points_used": hours * settings.HOST_COST_POINTS,
    ...
    async def restart_deployment(self, deployment_id: str) -> bool:
        ...
        expires_at = dep.get("expires_at", 0)
        if expires_at and time.time() > expires_at:
            terminal.add_error("Deployment has expired. Please renew with points.")
            await database.update_deployment(deployment_id, {"status": "expired"})
            return False
    ...
    async def delete_deployment(self, deployment_id: str) -> bool:
        ...
        try:
            ...
            # Update hosting history
            await database.update_hosting_status(deployment_id, "deleted")
