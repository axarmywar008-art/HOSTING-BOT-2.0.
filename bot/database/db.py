...
class Database:
    ...
    async def _create_indexes(self):
        ...
        await self.db.point_transactions.create_index([("user_id", 1), ("created_at", -1)])
        await self.db.deployment_points.create_index("deployment_id", unique=True)
    ...
    # ---- Points Management ----
    async def add_points(self, user_id: int, amount: int, reason: str = "admin_added"):
        """Add points to a user's balance."""
        user = await self.get_user(user_id)
        if not user:
            return False
        
        new_balance = user.get("points", 0) + amount
        ...
        # Log transaction
        await self.db.point_transactions.insert_one({
            "user_id": user_id,
            "amount": amount,
            "type": "credit",
            "reason": reason,
    ...
    async def deduct_points(self, user_id: int, amount: int, reason: str = "deployment_usage"):
    ...
    async def get_point_transactions(self, user_id: int, limit: int = 50):
        return await self.db.point_transactions.find(
            {"user_id": user_id}
        ).sort("created_at", -1).limit(limit).to_list(None)

    async def get_all_users_with_points(self):
        return await self.db.users.find({}, {"user_id": 1, "username": 1, "points": 1}).to_list(None)

    # ---- Deployment Points Tracking ----
    async def create_deployment_point_record(self, deployment_id: str, user_id: int, daily_cost: int = 2):
        """Create a point tracking record for a deployment."""
        record = {
            "deployment_id": deployment_id,
            "user_id": user_id,
            "daily_cost": daily_cost,
            "last_deduction": datetime.datetime.utcnow(),
            "next_deduction": datetime.datetime.utcnow() + datetime.timedelta(days=1),
            "points_remaining": await self.get_points_balance(user_id),
            "status": "active",
            "created_at": datetime.datetime.utcnow()
        }
        await self.db.deployment_points.insert_one(record)
        return record

    async def get_deployment_point_record(self, deployment_id: str):
        return await self.db.deployment_points.find_one({"deployment_id": deployment_id})

    async def update_deployment_point_record(self, deployment_id: str, data: dict):
        await self.db.deployment_points.update_one(
            {"deployment_id": deployment_id},
            {"$set": data}
        )

    async def get_all_active_deployment_points(self):
        return await self.db.deployment_points.find(
            {"status": "active"}
        ).to_list(None)

    async def mark_deployment_points_inactive(self, deployment_id: str):
        await self.db.deployment_points.update_one(
            {"deployment_id": deployment_id},
            {"$set": {"status": "inactive"}}
        )
    ...
    async def delete_deployment(self, deployment_id: str):
        ...
        await self.mark_deployment_points_inactive(deployment_id)
    ...
    async def add_log(self, log_type: str, data: dict):
        log = {"type": log_type, "data": data, "timestamp": datetime.datetime.utcnow()}
