...
async def view_user_handler(client: Client, message: Message):
    ...
    text = (
        ...
        f"<b>📦 Total Deployments:</b> {len(deployments)}\n\n"
    )
    
    if deployments:
        text += "<b>🤖 Deployments:</b>\n"
        for dep in deployments[-5:]:
            status_emoji = "🟢" if dep.get("status") == "running" else "🟡" if dep.get("status") == "deploying" else "🔴"
            expires = dep.get("expires_at", 0)
            expires_str = "Expired" if expires < time.time() else f"{int((expires - time.time()) / 3600)}h left"
            text += f"  {status_emoji} {dep.get('framework', 'Bot')} - {expires_str}\n"
            text += f"    ID: <code>{dep['deployment_id'][:8]}</code>\n"
