...
async def start_handler(client: Client, message: Message):
    ...
    if not db_user:
        ...
        if referred_by:
            await database.add_points(referred_by, settings.REFERRAL_BONUS, "referral")
    ...
    deployments = await database.get_all_user_deployments(user_id)
    active_count = len([d for d in deployments if d.get("status") == "running"])

    # Custom start message with points info
    start_text = (
        f"{START_CAPTION.format(user=user.first_name or 'user')}\n\n"
        f"<b>⭐ Points Balance:</b> {points}\n"
        f"<b>🤖 Active Bots:</b> {active_count}\n"
        f"<b>📌 Min Points for Deployment:</b> {settings.MINIMUM_POINTS_FOR_DEPLOYMENT}\n"
        f"<b>📉 Daily Cost per Bot:</b> {settings.DAILY_POINT_COST} points"
    )
...
async def points_handler(client: Client, message: Message):
    ...
    if transactions:
        ...
        for tx in transactions:
            ...
            created = tx.get("created_at", datetime.datetime.utcnow())
            text += f"{type_emoji} {amount} pts - {reason} ({created.strftime('%Y-%m-%d %H:%M')})\n"
    else:
